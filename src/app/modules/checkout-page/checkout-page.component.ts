...
//import {Toaster} from "ngx-toastr";
...
export class CheckoutPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.fb.group({
      amount: [(environment.subscriptionPrice).toFixed(2), [Validators.required, Validators.min(1), Validators.max(100000)]],
  ...
  async initPay(): Promise<any> {
    try {
      ...
      //TODO: tok_23213
      const { data } = await this.stripeService.generatePaymentMethod(token.id, this.username!);
      ...
      (      //this.toastr.open({text: 'Algo ocurrio mientras procesaba el pago', caption: 'ERROR', type: 'danger'})
        ...
        this.toastr.error(err.error.message, 'Algo ocurrio mientras procesaba el pago');
      }
    }
